...
#----------------------------------------------
#Generate the grids
RGRD = np.array(tmpradius)*1000.0
...
for i in range(Rpoints):
	for j in range(Latpoints):
		for k in range(Lonpoints):
			...
			DATA[i,k,j] = DATAVEC[counter]*100.0
...
#-----------------------------------------------------------
#Now make grids and interpolate 
#-----------------------------------------------------------

#This is the output grid: Currently we have the same number of points at the original grid, but this is not necessary
minrad = 3490.0
...
outfilename2  = 'ASPECT_tomoglobe_absolutetemps_mth1_%s.dat' %userinput.extracttype
